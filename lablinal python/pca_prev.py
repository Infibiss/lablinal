...
def gauss_solver(A: 'Matrix', B: 'Matrix') -> List['Matrix']:
    ...
    bc = B.num_columns  # Число столбцов в B

    # Создаем расширенную матрицу [A|B]
    C = Matrix(num_rows=n, num_cols=m + bc, accuracy=A.accuracy)

    for r in range(1, n + 1):
        for c in range(1, m + 1):
            C[r, c] = A[r, c]

        for c2 in range(1, bc + 1):
            C[r, m + c2] = B[r, c2]
    ...
    for r in range(rankA + 1, n + 1):
        zeroA = all(abs(C[r, c]) < 1e-7 for c in range(1, m + 1))
        if zeroA and any(abs(C[r, c]) > 1e-7 for c in range(m + 1, m + bc + 1)):
            raise ValueError("Система несовместна")

    # Обратный ход
    for r in range(rankA, 0, -1):
        lead_col = next((c for c in range(1, m + 1) if abs(C[r, c]) > 1e-7), None)
        if lead_col is None:
            continue
        for rr in range(r - 1, 0, -1):
            factor = C[rr, lead_col]
            if abs(factor) > 1e-7:
                for cc in range(lead_col, m + bc + 1):
    ...
    for c in range(1, m + 1):
        if rowid <= rankA and abs(C[rowid, c]) > 1e-7:
    ...
    if bc == 1:
        ...
        # Добавляем частное решение, если система совместна (B не нулевой или есть свободные переменные)
        if any(abs(B[i+1,1]) > 1e-7 for i in range(m)) or len(free_cols) > 0:
        ...
        for sc in free_cols:
            ...
            if any(abs(x) > 1e-7 for x in param_vec):
        ...
        raise NotImplementedError("Гаусс для матрицы B с несколькими столбцами пока не реализован")
...
def find_eigenvalues(C: 'Matrix', tol: float=1e-10) -> List[float]:
    """
    Вход:
    ...
    intervals = []
    k = 200 # Коэффициент разбиения отрезка
...
def find_eigenvectors(C: 'Matrix', eigenvalues: List[float]) -> List['Matrix']:
    ...
    for lam in eigenvalues:
        # Сформировываем (C - lam * E)
        Cl = Matrix(num_rows=m, num_cols=m, accuracy=C.accuracy, arr=C.list())
        for i in range(1, m + 1):
            Cl[i, i] = Cl[i, i] - lam
        # Решаем однородную СЛАУ Cl * x = 0
        B = Matrix(num_rows=m, num_cols=1, accuracy=C.accuracy, arr=[[0.0] for _ in range(m)])
        solutions = gauss_solver(Cl, B)
        print(solutions)
        # Возьмем первый ненулевой вектор из базиса ядра
        v = None
        for cand in solutions:
            norm = math.sqrt(sum(cand[i + 1, 1]**2 for i in range(m)))
            if norm > 1e-10:
                # Нормируем вектор
                for i in range(m):
                    cand[i + 1, 1] /= norm
                ...
                break
        if v is None:
            # Если нет свободных переменных, возвращаем стандартный базисный вектор
            v = Matrix(num_rows=m, num_cols=1, accuracy=C.accuracy)
            v[1, 1] = 1.0
        vectors.append(v)
    return vectors
...
def plot_pca_projection(X_proj: 'Matrix') -> Figure:
    """
    Вход: проекция данных X_proj (n×k)
    Выход: объект Figure из Matplotlib
    """
    fig, ax = plt.subplots()
    n = X_proj.num_rows
    k = X_proj.num_columns

    xs = [X_proj[i, 1] for i in range(1, n + 1)]
    ys = [X_proj[i, 2] for i in range(1, n + 1)]

    ax.plot(xs, ys, marker='o', linestyle='-', color='b')  # Линия с маркерами
    ax.set_xlabel("PC1")
    if k >= 2:
        ax.set_ylabel("PC2")
    else:
...
def reconstruction_error(X: 'Matrix', X_recon: 'Matrix') -> float:
    ...
    """
    if X.num_rows != X_recon.num_rows or X.num_columns != X_recon.num_columns:
